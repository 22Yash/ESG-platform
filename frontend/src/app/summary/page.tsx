...
import {
  Download, TrendingUp, Users, Zap, Factory, DollarSign,
  AlertCircle, CheckCircle, Loader, ChevronDown,
} from "lucide-react";
...
export default function SummaryPage() {
  ...
  useEffect(() => {
    const fetchESGData = async () => {
      try {
        ...
        if (!resData || resData.length === 0) {
          ...
          // Set selectedYear to the first year ONLY if not already selected
    ...
    fetchESGData();
  }, [selectedYear]); // include selectedYear to fix the eslint warning
  ...
  const handleExport = (format: "PDF" | "CSV") => {
    if (currentData && selectedYear) {
      exportESG(currentData, selectedYear, format, dashboardRef.current);
    }
  };
  ...
  return (
    ...
    >
      {/* Key Metrics, Charts, and Bars here... */}
      {/* Use the existing code for StatCards, LineChart, PieChart, BarCharts */}
      {/* All will automatically re-render when `selectedYear` changes */}
      <div
        ref={dashboardRef}
        className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
      >
                  ...
                  <div className="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span className="font-semibold">
                      {currentData.totalRevenue
                        ? `₹${(currentData.totalRevenue / 1000000).toFixed(
                            1
                          )}M`
                        : "N/A"}
                    </span>
